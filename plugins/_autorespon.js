    ...
    if (m.isGroup) {
        if (m.mentionedJid.includes(this.user.jid)) {
            await this.send2Button(m.chat,
                isBanned ? 'JemerBot tidak aktif' : banned ? 'kamu dibanned' : 'JemerBot aktif',
                '©JemerBot',
...
`.trim(), '©JemerBot', 'Pemilik Bot', ',owner', m)
    }
    ...
    if (set.autoupdatestatus) {
        if (new Date() * 1 - set.status > 1000) {
            ...
            await this.setStatus(`Aktif selama ${uptime} | Mode: ${set.self ? 'Private' : set.group ? 'Hanya Grup' : 'Publik'} | JamerBot`).catch(_ => _)
